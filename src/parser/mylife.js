...
exports.myLifeParser = async (req, res) => {
  let transaction;
  try {
    ...
    let get_scraping_fields = await connection.query(
      `SELECT * FROM rabbitmq_scraping WHERE queue_status = 'Processing' AND sites_id="3"
    ...
    let city = get_scraping_fields[0]["state"];
    let state = "FL";
    ...
    const url = `https://www.mylife.com/pub-multisearch.pubview?searchFirstName=${firstName}&searchLastName=${lastName}&searchLocation=${city}%2C+${state}&whyReg=peoplesearch&whySub=Member+Profile+Sub&pageType=ps`;
    ...
    let results = await page.evaluate(() => {
      ...
      let res = [];
      let input;
      const link2 = "https://www.mylife.com";

...
exports.abCheckParser = async (req, res) => {
  let transaction;
  try {
    ...
    let get_scraping_fields = await connection.query(
      `SELECT * FROM rabbitmq_scraping WHERE queue_status = 'Processing' AND sites_id="7"
    ...
    console.log("setting navigations etc.....");
    await page.setJavaScriptEnabled(true);
    await page.setDefaultNavigationTimeout(0);
    await page.setDefaultTimeout(30000);
    await page.setRequestInterception(true);
    await page.setUserAgent(UA);
    console.log("setting page view.....");
    ...
    console.log("entering in evaluate phase......");
    const results = await page.evaluate(() => {
      let titleNodeList = Array.from(
        document.querySelectorAll("#peoplelist2 > div.card")
      );
      let res = [];
      titleNodeList.map((td) => {
        ...
        const location = td
          ...
          .textContent.trim();
        const linkContent = td
          .querySelector("div > a.link-to-details")
          .getAttribute("href");
        const link =
          linkContent[0] === "/"
            ? "https://www.advancedbackgroundchecks.com" + linkContent
            : linkContent;

...
exports.instantCheckMate = async (req, res) => {
  let transaction;
  let browser;
  let page;
  try {
    ...
    let get_scraping_fields = await connection.query(
      `SELECT * FROM rabbitmq_scraping WHERE queue_status = 'Processing' AND sites_id="18"
    ...
    try {
      await page.goto(
        `https://www.instantcheckmate.com/results?firstName=${firstName}&middleInitial=&lastName=${lastName}&city=${city}&state=${state}`
      );
